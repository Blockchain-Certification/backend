...
async function verifyCertificateProof({
  ...
  try {
    ...
    // get the flattened hexadecimal representation of the Merkle tree layers
    const hexLayersFlat = tree.getHexLayersFlat();
  
    // specify the leaf node indices for which to generate the proof
    const paramsToShareIndex = [0, 2];
  
    // generate the proof for the specified leaf node indices
    const multiProof = tree.getMultiProof(hexLayersFlat, paramsToShareIndex);
  
    // verify the proof for the specified leaf node indices and the Merkle root
    const root = tree.getRoot();
    const isValid = tree.verifyMultiProof(multiProof, paramsToShareIndex, root);
