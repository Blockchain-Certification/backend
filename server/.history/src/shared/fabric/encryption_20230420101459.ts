...
async function generateMerkleTree(certData: any) {
  ...
  const mTreeLeaves = certDataArray.map((x) =>
...
async function generateDACProof(
  ...
  // const multiProof = mTree.getMultiProof(layers, paramsToShareIndex);

// create an array of data
const data = ['a', 'b', 'c', 'd'];

// create an array of SHA256 hashes for the data
const leaves = data.map((x) => crypto.SHA256(x).toString(crypto.enc.Hex));

// create a Merkle tree using the SHA256 hashes as leaves
const tree = new MerkleTree(leaves, crypto.SHA256);

// get the flattened array of SHA256 hash layers
const layers = tree.getLayersFlat().map((x) => x.toString('hex'));

// get the indices of the leaves for which to generate the multiproofs
const leafIndices = [0, 2];

// generate the multiproofs for the specified leaf indices
const multiProofs = tree.getMultiProof(layers, leafIndices);

// verify the multiproofs for the specified leaf indices
const root = tree.getRoot().toString('hex');
const leavesToVerify = [leaves[0], leaves[2]];
const isValid = tree.verifyMultiProof(root, leafIndices, leavesToVerify, leaves.length, multiProofs);
...
async function verifyCertificateProof({
  ...
  try {
    ...
    // get the flattened array of SHA256 hash layers
    const layers = tree.getLayersFlat().map((x: any) => x.toString('hex'));
