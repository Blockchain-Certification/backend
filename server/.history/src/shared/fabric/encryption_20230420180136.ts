...
async function generateDACProof(
  ...
  const multiProofs = mTree.getMultiProof(layers, ````);
  return multiProofs;
}
...
async function verifyCertificateProof({
  ...
  const disclosedDataHash: any = disclosedDataValues.map((x) => {
    const dataVerify: any = identifyDateTimeValues(x);
    return crypto.SHA256(dat).toString(crypto.enc.Hex);
  });
  console.log(disclosedDataValues);
...
function identifyDateTimeValues(character: string) {
