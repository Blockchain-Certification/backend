...
async function generateDACProof(
  ...
  const disclosedDataValues = sharedFields.map((field) => disclosedData[field]);
  const paramsToShareIndex = getParamsIndexArray(sharedFields, dacSchema.ordering);

  const mTreeRoot = mTree.getRoot();
  const disclosedDataHash: any = disclosedDataValues.map((x) => SHA256(x));
  const leafHashes = disclosedDataHash.map((hash: any) => Buffer.from(hash.toString(), 'hex'));

  const leavesCount = mTree.getLeafCount();
  const proofElements = mTree.generateMultiProof(paramsToShareIndex, leafHashes);

  const mTreeProof = proofElements.map((el) => el.toString('hex'));
