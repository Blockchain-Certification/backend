 ...
 * @param {Buffer[]} mTreeProof - Merkle tree proof
 * @param {Object} disclosedData - Key value pair containing the disclosed data. Eg - {"attributeName" : "attributeValue" }
 * @param {String} certUUID - Certificate UUID
 * @returns {Promise<boolean>}
 */
async function verifyCertificateProof(mTreeProof, disclosedData, certUUID) {
  // Query certificate schema
  const certSchema = await queryCertificateSchema("admin");
  
  // Get certificate data from the database
  const certificateDBData = await certificates.findOne({"_id": certUUID});

  // Generate Merkle tree from the certificate data
  const mTree = await generateMerkleTree(certificateDBData);

  // Extract disclosed data into separate arrays for names and values
  const disclosedDataParamNames = [];
  const disclosedDataValues = [];
  for (const field in disclosedData) {
    disclosedDataParamNames.push(field);
  ...
  // Get the indices of the disclosed parameters in the schema
  const paramsToShareIndex = getParamsIndexArray(
    disclosedDataParamNames,
    certSchema.ordering,
  );
  ...
  // Hash the disclosed data
  const disclosedDataHash = disclosedDataValues.map((x) => SHA256(x));
